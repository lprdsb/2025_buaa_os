...
int sys_env_destroyf(u_int envid) {
	struct Env *e;
...
int sys_exofork(void) {
	...
	e->pvar = curenv->pvar;
	// int r = sys_mem_map(curenv->env_id, curenv->pvar, e->env_id, e->pvar, PTE_D);
	// if(curenv->env_id == 0x2003){
		// printk("#### %08x  %d ####\n", curenv->pvar, r);
		// printk("#### %d ####\n", *((int*)curenv->pvar));
	// }
	// e->var_cnt[0] = 0;
	// e->var_cnt[1] = curenv->var_cnt[1];
	// for(int i = 0; i < curenv->var_cnt[1]; ++i){
	// 	strcpy(e->vars[1][i].name, curenv->vars[1][i].name);
	// 	strcpy(e->vars[1][i].val, curenv->vars[1][i].val);
	// 	e->vars[1][i].r = curenv->vars[1][i].r;
	// }

	return e->env_id;
}

u_int sys_getparentid(void){
...
// 		// printk("%s\n", name);
// 		// printk("%d\n", curenv->var_cnt[o]);
// 		for (int i = 0; i < e->var_cnt[o]; i++) {
// 			// printk("%08x\n", curenv->vars[o][i].name);
// 			if (strcmp(e->vars[o][i].name, name) == 0){
// 				// printk("@@@@ %s\n", e->vars[o][i].name);
// 				return &e->vars[o][i];
// 			}
// 		}
...
// u_int sys_setvar(char *name, char *val, int r, int x){
// 	struct Env *e;
// 	try(envid2env(curenv->env_parent_id, &e, 0));
...
// 		if (r) var->r = 1;
// 	} else {
// 		if (e->var_cnt[0] + e->var_cnt[1] >= 1024) {
// 			printk("declare: too many variables\n");
// 			return -1;
// 		}
// 		strcpy(e->vars[x][e->var_cnt[x]].name, name);
// 		strcpy(e->vars[x][e->var_cnt[x]].val, val ? val : "");
// 		e->vars[x][e->var_cnt[x]].r = r;
// 		e->var_cnt[x]++;
// 	}
// 	// debugf("%d\n", var_cnt);
// 	return 0;
...
void sys_varmap(u_int p){
	// curenv->pvar = p;
	sys_mem_map(curenv->env_parent_id, p, curenv->env_id, p, PTE_D | PTE_LIBRARY);
}

void sys_varinit(u_int p){
	curenv->pvar = p;
}
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_env_destroyf] = sys_env_destroyf,
	[SYS_exitcmd] = sys_exitcmd,
	// [SYS_setvar] = sys_setvar,
	// [SYS_findvar] = sys_findvar,sys_varinit
    [SYS_varinit] = sys_varinit,
    [SYS_varmap] = sys_varmap,
    [SYS_yield] = sys_yield,
    ...
    [SYS_set_env_relative_path] = sys_set_env_relative_path,
};
